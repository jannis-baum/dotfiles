...
let sketchytabsPort = null;

function connect() {
    sketchytabsPort = browser.runtime.connectNative('sketchytabs');

    sketchytabsPort.onMessage.addListener(async (message) => {
        const { switchTo, close } = message
        if (switchTo) {
            await browser.tabs.update(switchTo, { active: true });
        }
        if (close) {
            await browser.tabs.remove(close);
        }
    });

    sketchytabsPort.onDisconnect.addListener((p) => {
        if (p.error) console.error(`sketchytabs port disconnected: ${p.error.message}`);
        sketchytabsPort = null;
        setTimeout(() => { connect(); updateTabs(); }, 3000);
    });
}
...
async function updateTabs() {
    ...
    try {
        ...
        sketchytabsPort.postMessage(tabs)
    } catch (error) {
        console.error(error)
    }
}
...
events.forEach((event) => {
    event.addListener(() => {
        ...
        bugFixTimeout = setTimeout(updateTabs, 150)
    })
})
